...
public class GameView extends JFrame implements java.io.Serializable {
	...
	public GameView(Terminator end1) {
		...
		JLabel lblOptionName = new JLabel(optionNames[0]);
	...
	/*
	public void addRectangle(int layer, int y)
	{
		System.out.println(y);
		rect.add(new JLabel(new ImageIcon(this.getClass().getResource("StackerChocolateBar4.png"))));
		rect.get(rect.size()-1).setOpaque(true);
		rect.get(rect.size()-1).setBounds(340, y, 303, 50);
		rect.get(rect.size()-1).setVisible(true);
		rect.get(rect.size()-1).setVerticalAlignment(SwingConstants.BOTTOM);
		rect.get(rect.size()-1).setForeground(Color.CYAN);
		rect.get(rect.size()-1).setBackground(Color.BLACK);
		rect.get(rect.size()-1).setFont(new Font("Sylfaen", Font.BOLD, 30));
		
		gamePanel.add(rect.get(rect.size()-1));
		gamePanel.validate();
		gamePanel.repaint();
		pack();
	}
